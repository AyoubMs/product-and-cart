        ...
        <table class="cart-table">
          ...
          <tbody>
            <tr v-for="(quantity, key, i) in cart" :key="i">
              ...
              <!-- <td>${{ (quantity || 0) * getPrice(key) }}</td> -->
              <td>${{ unitTotal(quantity || 0, getPrice(key)) }}</td>
              <td class="center">
                <button
                  @click="removeAndFocus(key, i)"
  ...
  methods: {
    ...
    calculateTotal () {
      ...
      const total = Object.entries(this.cart).reduce((acc, curr, index) => {
        return acc + (curr[1] || 0) * this.getPrice(curr[0])
      }, 0)
      return total.toFixed(2)
    },
    removeAndFocus (key, index) {
